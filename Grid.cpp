...
void Grid::SetCell(const CellPosition&CP, GameObject *GO)
{
	int Vcell = CP.VCell();
	int Hcell = CP.HCell();
	CellList[Vcell][Hcell]->SetGameObject(GO);
}

Cell* Grid::GetCell(const CellPosition&CP) const
{
	int vCell = CP.VCell();
	int hCell = CP.HCell();
	return CellList[vCell][hCell];
}
